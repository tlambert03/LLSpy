...
class LLSdir(object):
    ...
    def __init__(self, path, fname_pattern=None, ditch_partial=True):
        ...
        if self.has_settings:
            ...
            self.settings = parse_settings(self.settings_files[0])
            self.date = self.settings.get('date')
            self.parameters.update(self.settings.get('params'))
    ...
    def get_otf(self, wave, otfpath=config.__OTFPATH__):
        ...
        mask = None
        if hasattr(self, 'settings') and 'mask' in self.settings:
            innerNA = self.settings['mask'].get('innerNA')
            outerNA = self.settings['mask'].get('outerNA')
    ...
    def correct_flash(self, tRange=None, camparamsPath=None, flashCorrectTarget='parallel',
        ...
        logger.debug("Correcting Flash artifact with camparam {}".format(camparams))
