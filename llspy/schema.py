...
__defaults__ = {
    ...
    "compressionType": (
        "lbzip2",
        "compression binary {lbzip2, bzip2, pbzip2, pigz, gzip}",
    ),
    "writeLog": (True, "write settings to processinglog.txt"),
    "padval": (0, "value to pad image with when deskewing"),
    "FlatStart": (
        False,
        "start decon from a flat image guess using the median image value",
    ),
    "dupRevStack": (
        False,
...
__validator__ = {
    ...
    "flashCorrectTarget": All(
        Coerce(str),
        Lower,
        Strip,
        Any("cpu", "parallel", "cuda"),
        msg="flashCorrectTarget must be {cpu, parallel, cuda}",
    ),
    "medianFilter": smartbool,
    "keepCorrected": smartbool,
    "trimZ": All(
        twotupIntRange, msg="trimZ argument must be a 2tuples of ints from 0-999"
    ),
    "trimY": All(
        twotupIntRange, msg="trimY argument must be a 2tuples of ints from 0-999"
    ),
    "trimX": All(
        twotupIntRange, msg="trimX argument must be a 2tuples of ints from 0-999"
    ),
    "nIters": All(
        Coerce(int),
        Range(0, 30),
    ...
    "reprocess": smartbool,
    "tRange": Any(
        None, CTiterable, msg="tRange must be int or iterable of integers >= 0"
    ),
    "cRange": Any(
        None, CTiterable, msg="cRange must be int or iterable of integers >= 0"
    ),
    "otfDir": Any(
        None, "", dirpath, msg="Unable to find OTF path. Check filepath in config"
    ),
    "camparamsPath": Any(
        None,
        filepath,
    ...
    "verbose": Any(0, 1, 2, msg="verbosity level must be 0, 1, or 2"),
