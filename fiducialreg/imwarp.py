...
def imwarp(inputImage, tform, R_A=None, outputRef=None):
	...
	outputImage = map_coordinates(inputImage, [srcZIntrinsic-1, srcYIntrinsic-1, srcXIntrinsic-1], order=1)
	return outputImage
...
def calculateOutputSpatialReferencing(R_in, tform):
	...
	R_out = snapWorldLimitsToSatisfyResolution(XWorldLimitsOut, YWorldLimitsOut,
		ZWorldLimitsOut, R_in.PixelExtentInWorldX, R_in.PixelExtentInWorldY,
		R_in.PixelExtentInWorldZ)
	return R_out


def snapWorldLimitsToSatisfyResolution(idealWLX, idealWLY, idealWLZ, outRX, outRY, outRZ):
